 ...
 *  @brief Shared code for CPU and GPU multi-pattern search benchmarks: the vocabulary, the corpus, and the arms.
 */
...
enum class substrings_slice_t {
    frequent_k, /**< The most common one percent, where nearly every byte enters an accepting state. */
    rare_k,     /**< The least common one percent, where the walk sits near the root and rarely reports. */
    sampled_k,  /**< Substrings of the corpus itself, which exist for any alphabet, with words or without. */
};
...
/** Shortest word admitted: anything under three bytes matches at nearly every position and measures the
 *  reporting path rather than the automaton. */
...
/** Occurrences a term needs to be admitted; a hapax is mostly OCR noise no haystack reaches twice. */
...
/** Distinct substrings of the haystacks, drawn at seeded random positions, so any corpus yields needles. */
 ...
 *
 *  Both noisy ends are removed before any slice is taken. The most frequent one percent are stopwords that
 *  every haystack holds, which would measure the reporting path rather than the automaton; terms occurring
 *  once are noise no haystack reaches twice. The top cutoff is a fraction of @b all distinct terms, hapax
 *  included, so the slice does not move when the hapax filter does.
 *
 *  Drawn from the dataset's words whatever tokenization shapes the haystacks, so a line search and a word
 *  search draw needles from the same vocabulary.
 */
...
struct substrings_dictionary_t {
    std::vector<std::string> needles;                   /**< The words this dictionary was compiled from. */
    std::vector<sz_string_view_t> needle_views;         /**< Views over @c needles, which only the builder reads. */
    unified_vector<char> replacement_bytes;             /**< Every replacement back to back, where a kernel reads. */
    unified_vector<sz_string_view_t> replacement_views; /**< One per needle, alternating with an empty deletion. */
    sz_sequence_t needle_sequence {};                   /**< Host accessors over @c needle_views. */
    sz_sequence_t replacements {};                      /**< Host accessors over @c replacement_views. */
    sz_memory_allocator_t allocator;                    /**< What every engine over this vocabulary is built from. */
    sz_substrings_case_sensitivity_t sensitivity;       /**< Whether both sides fold before they meet. */
    std::size_t needle_bytes = 0;                       /**< Needle bytes, which a compilation row is scaled by. */
...
enum class substrings_residency_t {
    host_k = 0,   /**< Plain host memory, walked by the CPU tiers. */
    device_k = 1, /**< Memory a kernel addresses, with the round's arena beside it. */
};
...
#if SZ_USE_CUDA
/** Compiles @p dictionary where a kernel reads it, through the unified allocator the dictionary carries. */
...
/** Joins the default stream when a device engine could have enqueued on it, which a host engine never does. */
...
/** A build with no device runtime has nothing to join, since every engine it builds is a host one. */
 ...
 *
 *  The policy sizes the arena, so it belongs to construction rather than to a call, and an arm therefore
 *  holds an engine rather than a vocabulary plus a policy argument.
 */
struct substrings_engine_t {
    sz_substrings_engine_t engine {}; /**< The compiled vocabulary, its arena and its report. */
...
/** The corpus as one sequence of haystacks, over views the device reaches when the build has one. */
struct substrings_corpus_t {
    unified_vector<sz_string_view_t> views; /**< One view per haystack, in corpus order. */
    sz_sequence_t haystacks {};             /**< Host accessors over @c views. */
    std::size_t bytes = 0;                  /**< Haystack bytes one round walks. */
...
struct substrings_counts_from_sz {
    sz_substrings_engine_t *engine;    /**< The compiled vocabulary this arm walks, policy included. */
    substrings_corpus_t const &corpus; /**< The haystacks one round walks. */
    sz_sequence_t haystacks;           /**< Host accessors for a CPU arm, device ones for a CUDA arm. */
    unified_vector<sz_size_t> counts;  /**< @b [haystacks], what a round fills. */
...
struct substrings_find_from_sz {
    ...
    substrings_corpus_t const &corpus;             /**< The haystacks one round walks. */
    sz_sequence_t haystacks;                       /**< Host accessors for a CPU arm, device ones for a CUDA arm. */
    unified_vector<sz_size_t> offsets;             /**< @b [haystacks + 1] boundaries into @c matches. */
    unified_vector<sz_substrings_match_t> matches; /**< Sized by a size query, so a round never grows it. */
...
struct substrings_replace_from_sz {
    ...
    substrings_corpus_t const &corpus; /**< The haystacks one round walks. */
    sz_sequence_t haystacks;           /**< Host accessors for a CPU arm, device ones for a CUDA arm. */
    sz_sequence_t replacements;        /**< Accessors over the replacements, of the same kind. */
    unified_vector<sz_size_t> offsets; /**< @b [haystacks + 1] rewritten boundaries, the last the total. */
    unified_vector<char> tape;         /**< Sized by a size query, so a round never grows it. */
...
struct substrings_bm25_from_sz {
    sz_substrings_engine_t *engine;    /**< The compiled vocabulary, which is the query. */
    substrings_corpus_t const &corpus; /**< The haystacks one round scores. */
    sz_sequence_t haystacks;           /**< Host accessors for a CPU arm, device ones for a CUDA arm. */
    sz_substrings_bm25_t parameters;   /**< The customary `k1` and `b`, over the corpus's mean bytes. */
    unified_vector<sz_f32_t> weights;  /**< @b [needles], all one: a weight scales a term, not the walk. */
    unified_vector<sz_f32_t> scores;   /**< @b [haystacks], what a round fills. */
