...
def test_slices():
    ...
    big = Str(native)
    assert native[0:5] == str(big.sub(0, 5)) and native[0:5] == str(big[0:5])
    assert native[5:10] == str(big.sub(5, 10)) and native[5:10] == str(big[5:10])

    assert native[5:5] == str(big.sub(5, 5)) and native[5:5] == str(big[5:5])
    assert native[-5:-5] == str(big.sub(-5, -5)) and native[-5:-5] == str(big[-5:-5])
    assert native[2:-2] == str(big.sub(2, -2)) and native[2:-2] == str(big[2:-2])
    assert native[7:-7] == str(big.sub(7, -7)) and native[7:-7] == str(big[7:-7])

    assert native[5:3] == str(big.sub(5, 3)) and native[5:3] == str(big[5:3])
    assert native[5:7] == str(big.sub(5, 7)) and native[5:7] == str(big[5:7])
    assert native[5:-3] == str(big.sub(5, -3)) and native[5:-3] == str(big[5:-3])
    assert native[5:-7] == str(big.sub(5, -7)) and native[5:-7] == str(big[5:-7])
    ...
    assert native[2:] == str(big.sub(2)) and native[2:] == str(big[2:])
    assert native[:7] == str(big.sub(end=7)) and native[:7] == str(big[:7])
    assert native[-2:] == str(big.sub(-2)) and native[-2:] == str(big[-2:])
    assert native[:-7] == str(big.sub(end=-7)) and native[:-7] == str(big[:-7])
    assert native[:-10] == str(big.sub(end=-10)) and native[:-10] == str(big[:-10])
    assert native[:-1] == str(big.sub(end=-1)) and native[:-1] == str(big[:-1])
