...
void test_find_unit() {
    ...
    // `sz_find` / `sz_rfind`: the substring "o" occurs at offsets 4 and 7.
    assert(sz_find(hello, hello_length, "o", 1) == hello + 4);         // Dispatched (automatic kernel)
    assert(sz_rfind(hello, hello_length, "o", 1) == hello + 7);        // Dispatched (automatic kernel)
    assert(sz_find_serial(hello, hello_length, "o", 1) == hello + 4);  // Manual propagation to the serial kernel
    assert(sz_rfind_serial(hello, hello_length, "o", 1) == hello + 7); // Manual propagation to the serial kernel
    // A multi-byte needle "wor" sits at offset 6, and a missing needle yields `SZ_NULL_CHAR`.
    assert(sz_find(hello, hello_length, "wor", 3) == hello + 6);          // Present multi-byte needle
    assert(sz_find(hello, hello_length, "xyz", 3) == SZ_NULL_CHAR);       // Missing needle
    assert(sz_rfind(hello, hello_length, "xyz", 3) == SZ_NULL_CHAR);      // Missing needle
    assert(sz_find_serial(hello, hello_length, "xyz", 3) == SZ_NULL_CHAR); // Missing needle, serial kernel
#if SZ_USE_WESTMERE
    assert(sz_find_westmere(hello, hello_length, "o", 1) == hello + 4);
    assert(sz_rfind_westmere(hello, hello_length, "o", 1) == hello + 7);
#endif
#if SZ_USE_HASWELL
    assert(sz_find_haswell(hello, hello_length, "o", 1) == hello + 4);
    assert(sz_rfind_haswell(hello, hello_length, "o", 1) == hello + 7);
#endif
#if SZ_USE_SKYLAKE
    assert(sz_find_skylake(hello, hello_length, "o", 1) == hello + 4);
    assert(sz_rfind_skylake(hello, hello_length, "o", 1) == hello + 7);
#endif
