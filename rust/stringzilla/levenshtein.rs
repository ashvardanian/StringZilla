...
///
/// Myers' bit-parallel masks are built once per batch and reused by every round, so the preparation a
/// one-shot distance repeats per pair is paid here exactly once. Construction also fixes the ISA tier
/// and, under `new_on_gpu`, the launch geometry.
///
/// The fields mirror `sz_levenshtein_engine_t` one for one and only `count` and `symbol` are read from
/// Rust, so the layout is load-bearing and the engine travels to C by pointer. Owning raw pointers makes it neither
/// `Send` nor `Sync`, which is what the C contract wants: a compute verb grows the engine's round
/// scratch, so it mutates, and every verb below takes `&mut self` for that reason.
///
/// # Examples
///
/// ```rust
...
impl LevenshteinEngine {
    ...
    ///
    /// Under [`LevenshteinSymbol::Runes`] the batch resolves to Skylake however capable the machine is,
    /// because Ice Lake's byte lanes have no rune arm.
    ...
    /// themselves are read on the host, so they need no device residency; everything the engine builds
    /// from them does.
    ///
    ...
    /// engine asynchronous: [`LevenshteinEngine::distances`] enqueues and returns, so its output slice
    /// has to be device-reachable, has to outlive the launch, and must not be read before the caller
    /// joins `stream` itself.
    #[cfg(feature = "cuda")]
    ...
    /// Edit distances from every prepared query to every candidate, on the tier the constructor fixed.
    ///
    /// `distances` receives a `[queries, candidates]` block: query `q` against candidate `c` lands at
    /// `distances[q * distances_stride + c]`, the stride counting entries rather than bytes and being
    /// at least the candidate count. The round's scratch grows here when a batch needs more than the
    /// last one did, and is never shrunk.
