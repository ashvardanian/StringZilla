 ...
 *  p8("sz_word_break_prop_ascii_", ascii_tbl, 16)
 *  p8("sz_word_break_prop_stage1_", stage1, 16)
 *  p8("sz_word_break_prop_stage2_", stage2, 16)
 *  print(f"#define SZ_TR29_WB_SMP_TRANSITION_COUNT_ {len(smp_trans)}")
 *  p32("sz_word_break_prop_smp_transitions_", smp_trans, 6)
 *  @endcode
 */

/* ASCII direct property table (128 bytes) - one property byte per codepoint */
static const sz_u8_t sz_word_break_prop_ascii_[128] = {
...
static const sz_u8_t sz_word_break_prop_stage1_[256] = {
...
static const sz_u8_t sz_word_break_prop_stage2_[6400] = {
...
static const sz_u32_t sz_word_break_prop_smp_transitions_[842] = {
...
SZ_PUBLIC sz_u8_t sz_rune_word_break_property(sz_rune_t rune) {
    ...
    if (rune < 0x80) { return sz_word_break_prop_ascii_[rune]; }
    ...
    if (rune < 0x10000) {
        sz_u8_t block_index = sz_word_break_prop_stage1_[rune >> 8];
        ...
        sz_u8_t packed = sz_word_break_prop_stage2_[stage2_offset + (low_byte >> 1)];
    ...
    while (search_low < search_high) {
        ...
        sz_rune_t trans_cp = sz_word_break_prop_smp_transitions_[mid] >> 4;
    ...
    return (sz_u8_t)(sz_word_break_prop_smp_transitions_[search_low - 1] & 0x0F);
}
 ...
 */
SZ_INTERNAL sz_bool_t sz_word_break_is_ignorable_(sz_u8_t prop) {
 ...
 */
SZ_INTERNAL sz_bool_t sz_word_break_is_ahletter_(sz_u8_t prop) {
 ...
 */
SZ_INTERNAL sz_bool_t sz_word_break_is_midnumletq_(sz_u8_t prop) {
...
SZ_INTERNAL sz_size_t sz_utf8_skip_ignorables_forward_(sz_cptr_t text, sz_size_t length, sz_size_t position) {
    while (position < length) {
        ...
        sz_u8_t prop = sz_rune_word_break_property(rune);
        if (!sz_word_break_is_ignorable_(prop)) break;
 ...
 */
SZ_INTERNAL sz_u8_t sz_word_break_get_effective_prop_(sz_cptr_t text, sz_size_t length, sz_size_t position,
    ...
    while (current < length && sz_word_break_is_ignorable_(prop)) {
 ...
 */
SZ_INTERNAL sz_u8_t sz_word_break_prev_prop_(sz_cptr_t text, sz_size_t position) {
    ...
    while (sz_word_break_is_ignorable_(prop) && previous > 0) {
        previous--;
        while (previous > 0 && ((sz_u8_t)text[previous] & 0xC0) == 0x80) previous--;
 ...
 */
SZ_INTERNAL sz_size_t sz_word_break_count_ri_before_(sz_cptr_t text, sz_size_t position) {
    sz_size_t count = 0;
    sz_size_t current = position;
    ...
    while (current > 0) {
        ...
        else if (sz_word_break_is_ignorable_(prop)) {
...
SZ_PUBLIC sz_bool_t sz_utf8_is_word_boundary_serial(sz_cptr_t text, sz_size_t length, sz_size_t position) {
    ...
    sz_u8_t prev_prop = sz_word_break_prev_prop_(text, position);
    ...
    if (sz_word_break_is_ignorable_(after_prop)) {
        sz_size_t skip_position = position;
        after_prop = sz_word_break_get_effective_prop_(text, length, skip_position, (sz_size_t *)0);
    }

    // WB5: Do not break between AHLetter
    if (sz_word_break_is_ahletter_(prev_prop) && sz_word_break_is_ahletter_(after_prop)) return sz_false_k;

    // WB6: Do not break AHLetter × (MidLetter|MidNumLetQ) × AHLetter
    if (sz_word_break_is_ahletter_(prev_prop) &&
        (after_prop == sz_tr29_word_break_midletter_k || sz_word_break_is_midnumletq_(after_prop))) {
        ...
        if (lookahead < length) {
            ...
            if (sz_word_break_is_ahletter_(la_prop)) return sz_false_k;
        }
    }

    // WB7: Do not break AHLetter (MidLetter|MidNumLetQ) × AHLetter
    if ((prev_prop == sz_tr29_word_break_midletter_k || sz_word_break_is_midnumletq_(prev_prop)) &&
        sz_word_break_is_ahletter_(after_prop)) {
        ...
        if (previous_cp_start > 0) {
            ...
            if (sz_word_break_is_ahletter_(pp_prop)) return sz_false_k;
        }
    }
    ...
    if (sz_word_break_is_ahletter_(prev_prop) && after_prop == sz_tr29_word_break_numeric_k) return sz_false_k;

    // WB10: Do not break Numeric × AHLetter
    if (prev_prop == sz_tr29_word_break_numeric_k && sz_word_break_is_ahletter_(after_prop)) return sz_false_k;

    // WB11: Do not break Numeric × (MidNum|MidNumLetQ) × Numeric
    if (prev_prop == sz_tr29_word_break_numeric_k &&
        (after_prop == sz_tr29_word_break_midnum_k || sz_word_break_is_midnumletq_(after_prop))) {
        sz_size_t lookahead = after_position;
        lookahead = sz_utf8_skip_ignorables_forward_(text, length, lookahead);
        if (lookahead < length) {
            sz_size_t la_position = lookahead;
    ...
    if ((prev_prop == sz_tr29_word_break_midnum_k || sz_word_break_is_midnumletq_(prev_prop)) &&
    ...
    if ((sz_word_break_is_ahletter_(prev_prop) || prev_prop == sz_tr29_word_break_numeric_k ||
    ...
    if (prev_prop == sz_tr29_word_break_extendnumlet_k &&
        (sz_word_break_is_ahletter_(after_prop) || after_prop == sz_tr29_word_break_numeric_k ||
    ...
    if (prev_prop == sz_tr29_word_break_regional_ind_k && after_prop == sz_tr29_word_break_regional_ind_k) {
        ...
        sz_size_t ri_count = sz_word_break_count_ri_before_(text, position);
