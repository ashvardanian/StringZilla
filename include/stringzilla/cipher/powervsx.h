 ...
 *
 *  The fourteen rounds are written out rather than looped, for the same reason the eight lanes are
 *  and the one `cipher/icelake.h` spells out.
 *
...
/*  Power is the only target here with both a native round instruction and a native carry-less
 *  multiply, and both of them read a register in big-endian order no matter which endianness the
 *  translation unit is built for. That is what the @c _be in @c vec_cipher_be and @c vec_pmsum_be
 *  means: the instruction is not endian-adjusted the way @c vec_perm and @c vec_splat are, so
 *  element zero of a little-endian vector is the @b last byte the round function sees.
 *
 *  Every sixteen-byte value in this file therefore lives in one single representation, called
 *  big-endian register order below: block byte zero occupies the most significant register byte.
 *  @c vec_xl_be and @c vec_xst_be move between memory and that order on both endiannesses in one
 *  instruction each, so the choice costs nothing, and the round keys, counter blocks, keystream,
 *  hash blocks and subkey powers all share it. Two consequences are worth spelling out.
 *
 *  The Galois hash needs @b no byte reflection here. A carry-less multiplier wants the field
 *  element's lowest coefficient in the most significant bit, which is exactly where the tag's byte
 *  order already puts it once the block is in big-endian register order; the byte shuffle that the
 *  x86 backends spend on every block is simply absent. The multiplier's remaining single-bit offset
 *  is still made up by the leftward shift inside the reduction.
 *
 *  Only the round-key array is genuinely endian-dependent, because @c sz_aes256_key_t stores words
 *  as @c sz_u32_t and the byte image of a word differs between the two targets. That one conversion
...
/**
 *  @brief Writes one round key back into an expanded schedule,
 *      undoing @ref sz_aes256_round_key_powervsx_.
 ...
 *
 *  The schedule's recurrence reads only the word that comes last, and @c vec_splat is one of the
...
/**
 *  @brief Rotates, substitutes and offsets a round key's last word, the
 *      schedule's round-constant step.
 *  @param[in] round_key_u8x16 The round key in big-endian register order.
 *  @param[in] round_constant The round constant for this step.
 *  @return The transformed word in every lane.
 *
 *  Broadcasting before rotating is what makes the rotation a single instruction: once all four
 ...
 *  @param[in] key The expanded schedule.
 *  @param[inout] blocks_u8x16 The eight plaintext blocks, replaced by their ciphertexts.
 *
 ...
 *  @param[in] input The sixteen input bytes.
...
STRINGZILLA_API_COMPTIME void sz_aes256_ctr_xor_powervsx(sz_aes256_key_t const *key,
    ...
    for (; produced + 8 * STRINGZILLA_AES_BLOCK_LENGTH <= length; produced += 8 * STRINGZILLA_AES_BLOCK_LENGTH) {
        ...
        sz_aes256_ctr_lane_powervsx_(input_bytes + produced + 0 * STRINGZILLA_AES_BLOCK_LENGTH,
                                     output_bytes + produced + 0 * STRINGZILLA_AES_BLOCK_LENGTH, keystream_u8x16[0]);
        sz_aes256_ctr_lane_powervsx_(input_bytes + produced + 1 * STRINGZILLA_AES_BLOCK_LENGTH,
                                     output_bytes + produced + 1 * STRINGZILLA_AES_BLOCK_LENGTH, keystream_u8x16[1]);
        sz_aes256_ctr_lane_powervsx_(input_bytes + produced + 2 * STRINGZILLA_AES_BLOCK_LENGTH,
                                     output_bytes + produced + 2 * STRINGZILLA_AES_BLOCK_LENGTH, keystream_u8x16[2]);
        sz_aes256_ctr_lane_powervsx_(input_bytes + produced + 3 * STRINGZILLA_AES_BLOCK_LENGTH,
                                     output_bytes + produced + 3 * STRINGZILLA_AES_BLOCK_LENGTH, keystream_u8x16[3]);
        sz_aes256_ctr_lane_powervsx_(input_bytes + produced + 4 * STRINGZILLA_AES_BLOCK_LENGTH,
                                     output_bytes + produced + 4 * STRINGZILLA_AES_BLOCK_LENGTH, keystream_u8x16[4]);
        sz_aes256_ctr_lane_powervsx_(input_bytes + produced + 5 * STRINGZILLA_AES_BLOCK_LENGTH,
                                     output_bytes + produced + 5 * STRINGZILLA_AES_BLOCK_LENGTH, keystream_u8x16[5]);
        sz_aes256_ctr_lane_powervsx_(input_bytes + produced + 6 * STRINGZILLA_AES_BLOCK_LENGTH,
                                     output_bytes + produced + 6 * STRINGZILLA_AES_BLOCK_LENGTH, keystream_u8x16[6]);
        sz_aes256_ctr_lane_powervsx_(input_bytes + produced + 7 * STRINGZILLA_AES_BLOCK_LENGTH,
                                     output_bytes + produced + 7 * STRINGZILLA_AES_BLOCK_LENGTH, keystream_u8x16[7]);
    }
 ...
 *  @param[in] block_u8x16 One operand, in big-endian register order.
 *  @param[in] subkey_u8x16 The other operand, one of the subkey powers.
 ...
 *  @param[in] low_u8x16 The accumulated low halves.
 *  @param[in] middle_u8x16 The accumulated cross products.
 *  @param[in] high_u8x16 The accumulated high halves.
 ...
 *  @param[in] accumulator_u8x16 The running hash.
 *  @param[in] block_u8x16 The block to absorb, in big-endian register order.
...
/**
 *  @brief Loads the eight subkey powers, descending, so power @c j pairs with block @c j
 *      of a group.
...
STRINGZILLA_HELPER_INLINE void sz_ghash_descending_powers_powervsx_(sz_u8_t const *powers,
    ...
    powers_u8x16[3] = sz_aes256_block_load_powervsx_(powers + 4 * STRINGZILLA_AES_BLOCK_LENGTH);
    powers_u8x16[4] = sz_aes256_block_load_powervsx_(powers + 3 * STRINGZILLA_AES_BLOCK_LENGTH);
    powers_u8x16[5] = sz_aes256_block_load_powervsx_(powers + 2 * STRINGZILLA_AES_BLOCK_LENGTH);
    powers_u8x16[6] = sz_aes256_block_load_powervsx_(powers + 1 * STRINGZILLA_AES_BLOCK_LENGTH);
    powers_u8x16[7] = sz_aes256_block_load_powervsx_(powers + 0 * STRINGZILLA_AES_BLOCK_LENGTH);
}

/**
 *  @brief Absorbs eight blocks into the running hash with a single field reduction.
 *  @param[in] accumulator_u8x16 The running hash.
 *  @param[in] blocks_u8x16 The eight blocks, in the order they arrived.
 *  @param[in] powers_u8x16 The powers H⁸ through H¹.
 *  @return The updated running hash.
 *
...
STRINGZILLA_HELPER_INLINE void sz_aes256_gcm_state_scrub_powervsx_(sz_aes256_gcm_state_t *state) {
    ...
    vec_xst(zero_u8x16, 0 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 1 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 2 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 3 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 4 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 5 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 6 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 7 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 8 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 9 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 10 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 11 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 12 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 13 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 14 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 15 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 16 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 17 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 18 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 19 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 20 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 21 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 22 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 23 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 24 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 25 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 26 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 27 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    vec_xst(zero_u8x16, 28 * STRINGZILLA_AES_BLOCK_LENGTH, bytes);
    for (byte_index = 29 * STRINGZILLA_AES_BLOCK_LENGTH; byte_index != sizeof(*state); ++byte_index)
        bytes[byte_index] = 0;
...
STRINGZILLA_HELPER_INLINE void sz_aes256_gcm_associate_powervsx_(sz_aes256_gcm_state_t *state, sz_cptr_t text,
    ...
    for (; consumed + 8 * STRINGZILLA_AES_BLOCK_LENGTH <= length; consumed += 8 * STRINGZILLA_AES_BLOCK_LENGTH) {
        ...
        blocks_u8x16[0] = sz_aes256_block_load_powervsx_(input_bytes + consumed + 0 * STRINGZILLA_AES_BLOCK_LENGTH);
        blocks_u8x16[1] = sz_aes256_block_load_powervsx_(input_bytes + consumed + 1 * STRINGZILLA_AES_BLOCK_LENGTH);
        blocks_u8x16[2] = sz_aes256_block_load_powervsx_(input_bytes + consumed + 2 * STRINGZILLA_AES_BLOCK_LENGTH);
        blocks_u8x16[3] = sz_aes256_block_load_powervsx_(input_bytes + consumed + 3 * STRINGZILLA_AES_BLOCK_LENGTH);
        blocks_u8x16[4] = sz_aes256_block_load_powervsx_(input_bytes + consumed + 4 * STRINGZILLA_AES_BLOCK_LENGTH);
        blocks_u8x16[5] = sz_aes256_block_load_powervsx_(input_bytes + consumed + 5 * STRINGZILLA_AES_BLOCK_LENGTH);
        blocks_u8x16[6] = sz_aes256_block_load_powervsx_(input_bytes + consumed + 6 * STRINGZILLA_AES_BLOCK_LENGTH);
        blocks_u8x16[7] = sz_aes256_block_load_powervsx_(input_bytes + consumed + 7 * STRINGZILLA_AES_BLOCK_LENGTH);
 ...
 *  @param[in] partial The pending hash bytes.
 *  @param[in] buffered How many of them are valid.
 *  @param[in] accumulator_u8x16 The running hash.
 *  @param[in] subkey_u8x16 The hash subkey.
 *  @return The updated running hash.
 *
 ...
 *  @param[out] output Receives the sixteen output bytes; may alias @p input.
 *  @param[in] keystream_u8x16 The keystream block.
 *  @param[in] cipher_mask_u8x16 All ones when decrypting, all zeros when encrypting.
...
STRINGZILLA_HELPER_INLINE void sz_aes256_gcm_transform_powervsx_(sz_aes256_gcm_state_t *state, sz_cptr_t text,
    ...
    for (; produced + 8 * STRINGZILLA_AES_BLOCK_LENGTH <= length; produced += 8 * STRINGZILLA_AES_BLOCK_LENGTH) {
        ...
        ciphertext_u8x16[0] = sz_aes256_gcm_lane_powervsx_(lane_input + 0 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           lane_output + 0 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           keystream_u8x16[0], cipher_mask_u8x16);
        ciphertext_u8x16[1] = sz_aes256_gcm_lane_powervsx_(lane_input + 1 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           lane_output + 1 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           keystream_u8x16[1], cipher_mask_u8x16);
        ciphertext_u8x16[2] = sz_aes256_gcm_lane_powervsx_(lane_input + 2 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           lane_output + 2 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           keystream_u8x16[2], cipher_mask_u8x16);
        ciphertext_u8x16[3] = sz_aes256_gcm_lane_powervsx_(lane_input + 3 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           lane_output + 3 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           keystream_u8x16[3], cipher_mask_u8x16);
        ciphertext_u8x16[4] = sz_aes256_gcm_lane_powervsx_(lane_input + 4 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           lane_output + 4 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           keystream_u8x16[4], cipher_mask_u8x16);
        ciphertext_u8x16[5] = sz_aes256_gcm_lane_powervsx_(lane_input + 5 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           lane_output + 5 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           keystream_u8x16[5], cipher_mask_u8x16);
        ciphertext_u8x16[6] = sz_aes256_gcm_lane_powervsx_(lane_input + 6 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           lane_output + 6 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           keystream_u8x16[6], cipher_mask_u8x16);
        ciphertext_u8x16[7] = sz_aes256_gcm_lane_powervsx_(lane_input + 7 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           lane_output + 7 * STRINGZILLA_AES_BLOCK_LENGTH,
                                                           keystream_u8x16[7], cipher_mask_u8x16);
