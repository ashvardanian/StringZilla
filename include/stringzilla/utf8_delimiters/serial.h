...
#include "stringzilla/utf8_runes.h"             // `sz_rune_parse`
 ...
 *
 *  Decodes each codepoint with the bounds-checked `sz_rune_parse`, so a truncated trailing UTF-8
...
SZ_PUBLIC sz_cptr_t sz_find_delimiters_utf8_serial(sz_cptr_t text, sz_size_t length, sz_size_t *matched_length) {
    ...
    while (position < end) {
        sz_rune_t rune;
        sz_rune_length_t const rune_length = sz_rune_parse(position, end, &rune);
        if (rune_length == sz_utf8_invalid_k) { position += 1; }
