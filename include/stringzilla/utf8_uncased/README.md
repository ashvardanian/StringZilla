...
This directory holds the kernels behind `sz_utf8_uncased_find`, which locates a needle inside a haystack while ignoring case differences.
...
Each table fixes one input shape; its single column is the `sz_utf8_uncased_find` operation and its rows are a backend on a chip, so reading down the column compares the backend ladder on one fixed input shape.
...
A `↑` cell means there is no dedicated `sz_utf8_uncased_find_<isa>` kernel at that backend, so the dispatcher reuses the tier above it.

## Short Words

| Backend          | `sz_utf8_uncased_find` |
| :--------------- | ---------------------: |
