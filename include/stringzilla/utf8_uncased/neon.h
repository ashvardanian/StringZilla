 ...
 *
 *  Ports the validated AVX2 architecture to 32-byte NEON chunks held as @c uint8x16x2_t, two
 *  16-byte registers mirroring a 32-byte YMM. The needle classification, match verification, and
 *  danger-zone scanning stay ISA-independent in `serial.h`; per-script fold and alarm helpers
 *  become @c uint8x16x2_t functions; and the shared force-inlined @c scripted_ driver walks the
 *  haystack in 32-byte steps. AVX2's two compare domains map directly: byte-mask vectors for folds,
 *  which feed masked adds, and a 32-bit movemask integer for alarms and probe filters, where scalar
 *  shifts replace @c VPMOVMSKB -shift algebra bit-for-bit. The 256-bit register has no NEON
 *  equivalent, so the fold callbacks splice the internal 16-byte boundary themselves with
 *  @c vextq_u8: the predecessor of register 1's lane 0 is register 0's lane 15, exactly as AVX2's
 *  @c VPERM2I128 carries the low lane across the 128-bit edge.
 */
 ...
 *
 *  Lane @c i of the low register sets bit @c i, lane @c j of the high register sets bit 16 + j.
 *  NEON has no @c VPMOVMSKB, so each register is AND-ed with the per-lane bit weights {1, 2, 4,
 *  ..., 128} repeated across the two 8-byte halves, then @c vaddv_u8 horizontally sums each half
 *  into its 8 packed bits. Unlike the @c vshrn nibble trick, with 4 bits per byte, this yields
...
/**
 *  @brief Shifts the 32 chunk bytes right by one lane, so lane @c i holds byte i − 1.
 *
 *  Lane 0 of the low register receives zero; lane 0 of the high register receives the low
 *  register's lane 15 - the carry across the internal 16-byte boundary. Vector-domain equivalent of
 *  Ice Lake's `k-mask << 1` and of AVX2's @c VPERM2I128 followed by @c VPALIGNR.
 *
 *  The zero fill at lane 0 is correct because a real match starts on a character boundary where the
 ...
 *
 *  @param[in] fold Script-specific 32-byte case-folding callback.
 ...
 *
 *  One @c vqtbl1q_u8 lookup over a 16-entry table replaces 3 range comparisons and 3 masked adds.
 ...
 *  Western European fold. The lead rewrites must cross the internal boundary, so the second-byte
