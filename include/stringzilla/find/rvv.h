...
SZ_PUBLIC sz_cptr_t sz_rfind_byte_rvv(sz_cptr_t haystack, sz_size_t haystack_length, sz_cptr_t needle) {
    ...
    while (haystack_length) {
        ...
        vbool2_t eq_mask = __riscv_vmseq_vx_u8m4_b2(reversed_u8m4, needle_byte, vl);
        long match_index = __riscv_vfirst_m_b2(eq_mask, vl);
...
SZ_PUBLIC sz_cptr_t sz_rfind_rvv(sz_cptr_t haystack, sz_size_t haystack_length, sz_cptr_t needle,
    ...
    while (remaining) {
        ...
        vuint8m4_t first_rev = sz_reverse_strip_rvv_(__riscv_vle8_v_u8m4(haystack_u8 + base + offset_first, vl), vl);
        vuint8m4_t mid_rev = sz_reverse_strip_rvv_(__riscv_vle8_v_u8m4(haystack_u8 + base + offset_mid, vl), vl);
        vuint8m4_t last_rev = sz_reverse_strip_rvv_(__riscv_vle8_v_u8m4(haystack_u8 + base + offset_last, vl), vl);
        vbool2_t first_mask = __riscv_vmseq_vx_u8m4_b2(first_rev, n_first, vl);
        vbool2_t mid_mask = __riscv_vmseq_vx_u8m4_b2(mid_rev, n_mid, vl);
        vbool2_t last_mask = __riscv_vmseq_vx_u8m4_b2(last_rev, n_last, vl);
        vbool2_t match_mask = __riscv_vmand_mm_b2(__riscv_vmand_mm_b2(first_mask, mid_mask, vl), last_mask, vl);
        // Walk reversed lanes low-to-high (= candidates high-to-low) via `vmsif`/`vmandn`.
        for (long reversed_index = __riscv_vfirst_m_b2(match_mask, vl); reversed_index >= 0;
             reversed_index = __riscv_vfirst_m_b2(match_mask, vl)) {
