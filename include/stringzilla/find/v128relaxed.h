...
/*  Byte and substring search are @c wasm_i8x16_eq, then @c wasm_i8x16_bitmask, then a
 *  trailing-zero count. The relaxed-SIMD instruction set, @c relaxed_swizzle,
 *  @c relaxed_laneselect, @c relaxed_madd and @c relaxed_nmadd, the integer and Q15
 *  @c relaxed_dot, @c relaxed_min and @c relaxed_max, and @c relaxed_trunc, operates on table
 *  lookups, lane muxing, fused multiply-add and float math. None of them act on an equality mask
 *  or movemask, so there is no relaxed way to make this search faster: @c find_byte,
 ...
 *  @c wasm_i8x16_relaxed_swizzle, whose index `c & 7` is always in [0, 7], so it can never hit the
 *  strict variant's out-of-range zeroing path and the result is identical to serial.
 *
 *  The two set lookups, over @c set_top and @c set_bottom, must stay on the strict
 *  @c wasm_i8x16_swizzle: they deliberately rely on the strict guarantee that out-of-range indices
 *  return 0 to implement the 32-byte table split across two 16-byte halves. Relaxed swizzle leaves
 *  those lanes undefined, so using it there would break correctness. */
