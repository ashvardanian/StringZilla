 ...
 *
 *  A batch is bucketed by rung key at @ref sz_levenshtein_engine_init_cuda, so one launch carries
 *  only queries that share an entry point, and the occupancy walk each of those entry points needs
 *  is paid there rather than once per round. The byte planes are built by a kernel from the queries
...
/** Query words one lane of the warped rung keeps verticals for; a warp carries thirty-two
 *  times as many. */
...
/** Query words the kernels reach together: a warp's thirty-two lanes at their widest. A query past
 *  this is refused rather than silently truncated. */
enum {
 ...
 *  one shared kernel would have to spend on every launch. @c sz_levenshtein_cuda_entry_point_
 *  picks between them. */
 ...
 *  stops at @c sz_levenshtein_cuda_candidates_per_block_max_k rather than at what the entry point
 ...
 *  step earlier and arrives through a single @c __shfl_up_sync. Unskewed, that same carry is a
...
/** Query symbols one byte lane of a Myers word holds, which is the longest query the
 *  byte rung takes. */
enum { sz_levenshtein_cuda_byte_lanes_symbols_max_k = 8 };

/** Query symbols one sixteen-bit lane holds, which is the longest query either
 *  narrow rung takes. */
 ...
 *  rune per step and hands its class up through a second @c __shfl_up_sync, so the class walks one
...
typedef struct sz_levenshtein_cuda_head_t {
    ...
    /** The @c cudaStream_t every round of this engine is scheduled on. */
    void *stream;
...
/**
 *  @brief Moves a host-filled block to the device in one migration, on the stream that is about
 *      to read it.
 *
 *  Managed pages sit wherever their last toucher left them, so a kernel reading a table the host
 *  has just filled faults it in a page at a time - 1,732 microseconds against 225 for the bulk move
 *  at the widest query this ladder admits. Memory the driver does not manage is not migratable and
 *  reports as much, which is not an error.
 */
 ...
 *      writes before it.
 *
 *  The release is spelled in PTX because the library's C tiers cannot reach @c cuda::atomic_ref,
 *  and because the alternative - a @c __threadfence next to a @c volatile store - orders every
 *  prior access of the thread where one location's release is all the protocol needs, and costs a
 ...
 *  column and top-right corner from @c __shfl_up_sync, which is itself the warp-wide rendezvous
...
/** Blocks of the wavefront the device holds at once; every launched block has to be resident, since
 *  a block spins on a tile-column another block owns. */
