 ...
 *  Public Core API:
 *
 *  - @c sz_utf8_uncased_search - uncased substring search in UTF-8 strings
 *  - @c sz_utf8_uncased_order - uncased lexicographical comparison of UTF-8 strings
 *  - @c sz_utf8_find_cased - pointer to the first cased (foldable) codepoint, or NULL if caseless
 *
 ...
 *  folding than the proper @c sz_utf8_uncased_fold, but without losing any possible matches. That's
 *  typically achieved finding parts of the needle, that never appear in any multi-byte expansions
 *  of complex characters, so we don't need to shuffle data within a CPU register - just swap some
 *  byte sequences with others.
 *
 ...
 *  letter case. Implements the same full Unicode Case Folding as @c sz_utf8_uncased_fold, including
 *  all one-to-many expansions (e.g. 'ß' (U+00DF, C3 9F) → "ss" (U+0073 U+0073, 73 73)) and
 *  bicameral script mappings.
 *
 ...
 *  case-agnostic, then @c sz_find can be used directly instead of the slower
 *  @c sz_utf8_uncased_search. This is particularly valuable for:
 *
 ...
 *          result = sz_utf8_uncased_search(haystack, haystack_length,
 ...
 *  (e.g., via the ICU library) and subsequently use the simpler @c sz_find function for repeated
 *  searches. This avoids the cost of performing full folding logic during every search operation.
 *
