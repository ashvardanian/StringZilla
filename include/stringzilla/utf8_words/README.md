...
This directory holds the kernels behind `sz_utf8_words`, which walks a UTF-8 string and yields each word as defined by the Unicode UAX-29 word-boundary rules, so "don't" or a CJK run is split the way a human reader expects rather than on raw spaces.
...
Each table fixes one input shape; its single column is the `sz_utf8_words` operation and its rows are a backend on a chip, so reading down the column compares the backend ladder on one fixed input shape.
