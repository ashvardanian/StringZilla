...
SZ_PUBLIC sz_bool_t sz_equal_powervsx(sz_cptr_t a, sz_cptr_t b, sz_size_t length) {
    ...
    __vector unsigned char a_u8x16, b_u8x16;
    sz_size_t offset = 0;
    do {
        a_u8x16 = vec_xl(0, (unsigned char const *)(a + offset));
        b_u8x16 = vec_xl(0, (unsigned char const *)(b + offset));
        ...
        if (vec_first_mismatch_index(a_u8x16, b_u8x16) != 16) return sz_false_k;
#else
        if (!vec_all_eq(a_u8x16, b_u8x16)) return sz_false_k; // True only when every byte matched.
#endif
        offset += 16;
    } while (offset + 16 <= length);

    // Final check - load the last register-long window from the end.
    a_u8x16 = vec_xl(0, (unsigned char const *)(a + length - 16));
    b_u8x16 = vec_xl(0, (unsigned char const *)(b + length - 16));
#if defined(_ARCH_PWR10)
    if (vec_first_mismatch_index(a_u8x16, b_u8x16) != 16) return sz_false_k;
#else
    if (!vec_all_eq(a_u8x16, b_u8x16)) return sz_false_k;
#endif
    return sz_true_k;
}
