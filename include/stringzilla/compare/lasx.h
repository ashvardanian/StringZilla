...
SZ_PUBLIC sz_bool_t sz_equal_lasx(sz_cptr_t a, sz_cptr_t b, sz_size_t length) {
    ...
    // instead of leaving half of a 256-bit LASX register idle.
        ...
        __m128i first_matches_vec = __lsx_vseq_b(a_first_vec.lsx, b_first_vec.lsx);
        __m128i second_matches_vec = __lsx_vseq_b(a_second_vec.lsx, b_second_vec.lsx);
        __m128i both_matches_vec = __lsx_vand_v(first_matches_vec, second_matches_vec);
        return (sz_bool_t)(sz_vmovemask_b_compare_lsx_(both_matches_vec) == 0xFFFFu);
    }
    // We can use 2x 256-bit interleaving loads, similar to the AVX2 backend, to handle up to 64 bytes.
        ...
        __m256i first_matches_vec = __lasx_xvseq_b(a_first_vec.lasx, b_first_vec.lasx);
        __m256i second_matches_vec = __lasx_xvseq_b(a_second_vec.lasx, b_second_vec.lasx);
        __m256i both_matches_vec = __lasx_xvand_v(first_matches_vec, second_matches_vec);
        return (sz_bool_t)(sz_xvmovemask_b_compare_lasx_(both_matches_vec) == 0xFFFFFFFFu);
    }
    else {
        sz_size_t i = 0;
        sz_u256_vec_t a_vec, b_vec;
        do {
            a_vec.lasx = __lasx_xvld(a + i, 0);
            b_vec.lasx = __lasx_xvld(b + i, 0);
