...
typedef struct sz_aes256_key_t {
    ...
    sz_u32_t round_keys[SZ_AES256_ROUND_KEYS];
} sz_aes256_key_t;
...
typedef struct sz_aes256_gcm_key_t {
    ...
    sz_u8_t powers[8 * SZ_AES_BLOCK_LENGTH];
} sz_aes256_gcm_key_t;
 ...
 *  @param[in] nonce The 12 nonce bytes, which must never repeat under one key.
 *  @param[in] associated Authenticated, unencrypted bytes like a routing header, or @c SZ_NULL.
 ...
 *  @param[in] associated Bytes authenticated but not encrypted, or @c SZ_NULL.
 *  @param[in] associated_length Number of associated bytes, possibly zero.
 ...
 *  backends - set @c SZ_DYNAMIC_DISPATCH to 1. */
