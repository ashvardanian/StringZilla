 ...
 *  - `sz_utf8_unpack_upto64` - convert UTF-8 to UTF-32 in a streaming manner
 *
 ...
 *  @param[out] runes Output buffer for UTF-32 codepoints (must have at least 64 entries).
 *  @param[out] unpacked_runes Number of runes unpacked.
 *  @return Pointer to the byte after the last unpacked byte in @p text.
 */
SZ_DYNAMIC sz_cptr_t sz_utf8_unpack_upto64( //
    sz_cptr_t text, sz_size_t length,       //
    sz_rune_t *runes, sz_size_t *unpacked_runes);
...
SZ_PUBLIC sz_cptr_t sz_utf8_unpack_upto64_serial( //
    sz_cptr_t text, sz_size_t length,             //
    sz_rune_t *runes, sz_size_t *unpacked_runes);
...
SZ_PUBLIC sz_cptr_t sz_utf8_unpack_upto64_haswell( //
    sz_cptr_t text, sz_size_t length,              //
    sz_rune_t *runes, sz_size_t *unpacked_runes);
...
SZ_PUBLIC sz_cptr_t sz_utf8_unpack_upto64_ice( //
    sz_cptr_t text, sz_size_t length,          //
    sz_rune_t *runes, sz_size_t *unpacked_runes);
...
SZ_PUBLIC sz_cptr_t sz_utf8_unpack_upto64_neon( //
    sz_cptr_t text, sz_size_t length,           //
    sz_rune_t *runes, sz_size_t *unpacked_runes);
...
SZ_PUBLIC sz_cptr_t sz_utf8_unpack_upto64_serial( //
    sz_cptr_t text, sz_size_t length,             //
    sz_rune_t *runes, sz_size_t *unpacked_runes) {
    sz_cptr_t const end = text + sz_min_of_two(length, 64);
    sz_size_t count = 0;
    sz_rune_length_t rune_length;
    for (; text != end; text += rune_length, runes++, count++) sz_rune_parse(text, runes, &rune_length);
    *unpacked_runes = count;
    return text;
}
...
SZ_PUBLIC sz_cptr_t sz_utf8_unpack_upto64_ice( //
    ...
    if (ascii_prefix_length) {
        ...
        sz_size_t runes_to_place = sz_min_of_two(ascii_prefix_length, 16);
        runes_vec.zmm = _mm512_cvtepu8_epi32(_mm512_castsi512_si128(text_vec.zmm));
        _mm512_mask_store_epi32(runes, sz_u16_mask_until_(runes_to_place), runes_vec.zmm);
        *unpacked_runes = runes_to_place;
    ...
    if (two_byte_prefix_length) {
        ...
        sz_size_t runes_to_place = sz_min_of_two(two_byte_prefix_length, 32);
        ...
        _mm512_mask_store_epi32(runes, sz_u32_mask_until_(runes_to_place), runes_vec.zmm);
        *unpacked_runes = runes_to_place;
    ...
    if (three_byte_prefix_length) {
        ...
        sz_size_t runes_to_place = sz_min_of_two(three_byte_prefix_length, 16);
        ...
        runes_vec.zmm = _mm512_or_si512(
            _mm512_or_si512(_mm512_slli_epi32(_mm512_and_si512(gathered_triplets.zmm, _mm512_set1_epi32(0x0FU)),
                                              12), // (b0 & 0x0F) << 12
                            _mm512_slli_epi32(
                                _mm512_and_si512(_mm512_srli_epi32(gathered_triplets.zmm, 8), _mm512_set1_epi32(0x3FU)),
                                6)), // (b1 & 0x3F) << 6
    ...
    if (four_byte_prefix_length) {
        ...
        sz_size_t runes_to_place = sz_min_of_two(four_byte_prefix_length, 16);
        // Decode: ((b0 & 0x07) << 18) | ((b1 & 0x3F) << 12) | ((b2 & 0x3F) << 6) | (b3 & 0x3F)
        // text_vec has: [b0, b1, b2, b3] in each 32-bit slot (little-endian: 0xb3b2b1b0)
        // Extract: b0 from bits 7-0, b1 from bits 15-8, b2 from bits 23-16, b3 from bits 31-24
        runes_vec.zmm = _mm512_or_si512(
            _mm512_or_si512(
                _mm512_slli_epi32(_mm512_and_si512(text_vec.zmm, _mm512_set1_epi32(0x07U)), 18), // (b0 & 0x07) << 18
                _mm512_slli_epi32(_mm512_and_si512(_mm512_srli_epi32(text_vec.zmm, 8), _mm512_set1_epi32(0x3FU)),
                                  12)), // (b1 & 0x3F) << 12
            _mm512_or_si512(
                _mm512_slli_epi32(_mm512_and_si512(_mm512_srli_epi32(text_vec.zmm, 16), _mm512_set1_epi32(0x3FU)),
                                  6),                                                              // (b2 & 0x3F) << 6
                _mm512_and_si512(_mm512_srli_epi32(text_vec.zmm, 24), _mm512_set1_epi32(0x3FU)))); // (b3 & 0x3F)
        _mm512_mask_store_epi32(runes, sz_u16_mask_until_(runes_to_place), runes_vec.zmm);
        *unpacked_runes = runes_to_place;
