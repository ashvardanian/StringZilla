...
STRINGZILLA_HELPER_AUTO sz_u32_t sz_substrings_step(sz_substrings_engine_t const *engine, sz_u32_t state,
    ...
    for (;;) {
        ...
        // `STRINGZILLA_U8_MAX` slots of headroom past the last state for exactly this reach.
...
STRINGZILLA_HELPER_AUTO sz_substrings_resolved_match_t sz_substrings_resolve_match(
    ...
    if (start_here != STRINGZILLA_NULL) resolved.source_offset = (sz_size_t)(start_here - haystack);
    resolved.repeats = (sz_bool_t)(periodic && start_here != STRINGZILLA_NULL && start_here == start_earlier);
    return resolved;
}
...
typedef struct sz_substrings_builder_t {
    ...
    sz_u8_t byte_to_class[STRINGZILLA_U8_MAX + 1];
...
STRINGZILLA_API_COMPTIME void sz_substrings_builder_free_(sz_substrings_builder_t *builder) {
    ...
    builder->order_scratch = STRINGZILLA_NULL;
    builder->root_row = STRINGZILLA_NULL, builder->base = STRINGZILLA_NULL, builder->check = STRINGZILLA_NULL;
    builder->state_of_slot = STRINGZILLA_NULL, builder->occupied = STRINGZILLA_NULL;
}
...
STRINGZILLA_API_COMPTIME void sz_substrings_builder_order_band_(sz_substrings_builder_t *builder, sz_size_t band_first,
    ...
    for (degree = 0; degree <= STRINGZILLA_U8_MAX + 1; ++degree) histogram[degree] = 0;
    ...
    for (degree = STRINGZILLA_U8_MAX + 2; degree-- > 0;) {
...
STRINGZILLA_API_COMPTIME sz_status_t sz_substrings_builder_pack_cold_children_(sz_substrings_builder_t *builder,
    ...
    for (candidate = first_free;;) {
        ...
        status = sz_substrings_builder_reserve_slots_(builder, candidate_base + STRINGZILLA_U8_MAX + 1);
        if (status != sz_success_k) return status;
...
STRINGZILLA_API_COMPTIME sz_status_t sz_substrings_builder_pack_(sz_substrings_builder_t *builder) {
    ...
    status = sz_substrings_builder_reserve_slots_(builder, builder->hot_count + STRINGZILLA_U8_MAX + 1);
    if (status != sz_success_k) return status;
...
STRINGZILLA_API_COMPTIME sz_substrings_layout_t sz_substrings_publish_layout_(sz_size_t hot_count,
    ...
    layout.byte_to_class = amount, amount += (STRINGZILLA_U8_MAX + 1) * sizeof(sz_u8_t);
    layout.total = amount;
    return layout;
}
...
STRINGZILLA_API_COMPTIME void sz_substrings_builder_classify_(sz_substrings_builder_t *builder) {
    ...
    for (byte = 0; byte != STRINGZILLA_U8_MAX + 1; ++byte) {
        if (!labelled[byte]) continue;
        builder->byte_to_class[byte] = (sz_u8_t)builder->classes_count++;
    }
    for (byte = 0; byte != STRINGZILLA_U8_MAX + 1; ++byte) {
        if (labelled[byte]) continue;
        if (shared_class == STRINGZILLA_U8_MAX + 1) shared_class = builder->classes_count++;
...
STRINGZILLA_API_COMPTIME void sz_substrings_engine_free_(sz_substrings_engine_t *engine) {
    ...
    engine->fail = STRINGZILLA_NULL;
    engine->byte_to_class = STRINGZILLA_NULL, engine->classes_count = 0;
    engine->accepts_words = STRINGZILLA_NULL, engine->outputs = STRINGZILLA_NULL;
    engine->outputs_counts = STRINGZILLA_NULL, engine->outputs_offsets = STRINGZILLA_NULL;
...
STRINGZILLA_API_COMPTIME sz_status_t sz_substrings_engine_compile_(
    ...
    builder.order = STRINGZILLA_NULL, builder.order_scratch = STRINGZILLA_NULL, builder.root_row = STRINGZILLA_NULL;
    builder.base = STRINGZILLA_NULL, builder.check = STRINGZILLA_NULL, builder.state_of_slot = STRINGZILLA_NULL,
    builder.occupied = STRINGZILLA_NULL;
...

    {
        ...
        for (slot = 0; slot != STRINGZILLA_U8_MAX + 1; ++slot) byte_to_class[slot] = builder.byte_to_class[slot];
...
typedef struct sz_substrings_collector_t {
    ...
    /** Where the matches land, or @c STRINGZILLA_NULL for a pure size query. */
...
typedef struct sz_substrings_rewriter_t {
    ...
    /** Where the rewrite lands, or @c STRINGZILLA_NULL for a pure size query. */
    sz_ptr_t output;
...
STRINGZILLA_API_COMPTIME sz_u32_t *sz_substrings_sort_needles_(sz_u32_t *keys, sz_size_t count,
    ...
    for (shift = 0; shift < 32 && (needles_count - 1) >> shift; shift += 8) {
        for (bucket = 0; bucket != STRINGZILLA_U8_MAX + 1; ++bucket) buckets[bucket] = 0;
        for (index = 0; index != count; ++index) ++buckets[(keys[index] >> shift) & STRINGZILLA_U8_MAX];
        for (bucket = 0, running = 0; bucket != STRINGZILLA_U8_MAX + 1; ++bucket) {
            sz_size_t const size = buckets[bucket];
            buckets[bucket] = running, running += size;
        }
        for (index = 0; index != count; ++index)
            spare[buckets[(keys[index] >> shift) & STRINGZILLA_U8_MAX]++] = keys[index];
        {
