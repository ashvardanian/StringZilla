/**
 *  @brief Window overlap between a batch of prepared queries and a batch of candidates, at one or more n-gram widths.
 ...
 *  windows that occur in the other, over whichever of the two has more. Every query is hashed once into one
 *  B-tree over every width's window hashes, and every candidate then streams through the forest, hashing its own
 *  windows and probing. Nothing is stored per candidate, so the candidates may change round to round while the
 *  queries and the widths stay.
 *
 ...
 *
 *  Every width's window hashes of one query share that query's B-tree, so its sort runs once per batch rather
 *  than once per round, and the probe depth grows only logarithmically with the widths added.
 *
 *  @param[in] queries The texts whose windows are sorted into the forest; read on the host.
 *  @param[in] window_widths Window widths in bytes; a width past a text scores zero for every pair it spans.
 *  @param[in] window_widths_count Number of widths, which is the last axis of every output.
 *  @param[in] alloc Where the forest's block comes from, or @c SZ_NULL for the default host allocator; stored by
 *      value, so @ref sz_overlap_engine_free needs no allocator of its own.
 *  @param[out] engine Left untouched unless the call succeeds.
 *
 *  @retval sz_unexpected_dimensions_k for zero widths.
 *  @retval sz_bad_alloc_k when the forest's block cannot be taken.
 *  @note Selects the fastest implementation at compile- or run-time based on @c SZ_DYNAMIC_DISPATCH, and records
 *      it, so a round never resolves a tier again.
 *  @sa sz_overlap_engine_init_serial, sz_overlap_engine_init_haswell, sz_overlap_engine_init_skylake
 */
 ...
 *  @param[out] engine Left untouched unless the call succeeds.
 *
 *  @retval sz_unexpected_dimensions_k for zero widths, or for a width the device backend's per-thread ring cannot
 *      hold - see @ref sz_overlap_cuda_widest_window_k and @ref sz_overlap_cuda_widths_max_k.
 *  @retval sz_device_memory_mismatch_k when @p alloc hands back memory the device cannot reach.
 *  @retval sz_device_code_mismatch_k when no GPU runtime is compiled in.
 ...
 *
 *  @param[in] engine A batch prepared by @ref sz_overlap_engine_init_cpu or @ref sz_overlap_engine_init_gpu, which
 *      also picked the tier this runs on; its round block grows here and is never shrunk.
 *  @param[in] candidates The texts whose windows are probed against the forest's.
 *  @param[out] scores Share @c [q,c,w] at @c scores[q*scores_query_stride + c*scores_candidate_stride + w], each
 *      in @c [0,1]; the width axis is unit-strided.
 *  @param[in] scores_query_stride Entries from one query's plane to the next, at least candidates times the below.
 *  @param[in] scores_candidate_stride Entries from one candidate's row to the next, at least the engine's widths.
 *
 *  @retval sz_unexpected_dimensions_k when either stride is under the axis it spans.
 *  @retval sz_bad_alloc_k when the round's block cannot be grown.
 *  @note Asymmetric: a candidate's window occurrences count against a query's distinct windows, so swapping
 *      sides changes the score when either repeats a window.
 *  @note On a device engine this enqueues and returns; the caller joins the stream before reading @p scores.
 *  @sa sz_overlap_scores_serial, sz_overlap_scores_haswell, sz_overlap_scores_skylake, sz_overlap_scores_cuda
 */
