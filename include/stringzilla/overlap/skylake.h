/**
 *  @brief AVX-512 (Skylake) backend for window overlap: eight prefix hashes from one word, the window hashes eight
 *      positions at a time, the sort over sixteen-key registers, and the B-tree probe sixteen keys at a time.
...
/** Eight positions' window hashes: @c H(i,w) = P(i+w) - P(i)·b^w, a full-width hash under the modulus. */
...
/** Positions @c i ^ @p flip for the sixteen keys of a register: the partner at a distance, or across a mirror. */
...
/** Compare-exchange inside a register: every key against the one @p partners_vec names, masked positions take the
 *  larger. */
...
/** Sorts the sixteen keys of one register ascending: the merges of two, four, eight and sixteen keys, all inside it. */
...
/** The ascending half-cleaners at distances eight, four, two and one, closing a merge inside the register. */
...
/** The ascending stage pairing each of @c registers_vec[0…8) with the register eight past it. */
...
/** The ascending stage pairing each of @c registers_vec[0…4) with the register four past it. */
...
/** Sorts one hundred twenty-eight keys in eight registers, in place: two sorted quads, then their merge. */
...
/** Closes a merge inside one 256-key run: the ascending stages at distances one hundred twenty-eight down to one. */
...
SZ_API_COMPTIME sz_status_t sz_overlap_scores_skylake(sz_cptr_t query, sz_size_t query_length,
    ...
    for (; position + step <= query_length; position += step)
        prior = sz_overlap_f64x8_prefix_hash_step_skylake(prior, query + position, query_prefix_hashes + position + 1);
    if (position != query_length)
        sz_overlap_f64x8_prefix_hash_step_tail_skylake(prior, query + position, query_length - position,
    ...
    for (sz_size_t width_index = 0; width_index != window_widths_count; ++width_index) {
        ...
        if (window != query_windows)
            sz_overlap_f64x8_window_hash_step_tail_skylake(query_prefix_hashes + window,
    ...
    sz_overlap_btree_prepare(nodes, sz_overlap_u32x16_btree_sort_skylake(nodes, written), &btree);
...
SZ_API_COMPTIME sz_status_t sz_overlap_score_skylake(sz_cptr_t query, sz_size_t query_length, sz_cptr_t candidate,
    ...
    for (; walked + step <= shorter; walked += step) {
        query_prior = sz_overlap_f64x8_prefix_hash_step_skylake(query_prior, query + walked,
                                                                query_prefix_hashes + walked + 1);
        candidate_prior = sz_overlap_f64x8_prefix_hash_step_skylake(candidate_prior, candidate + walked,
                                                                    candidate_prefix_hashes + walked + 1);
    }
    sz_size_t position = walked;
    for (; position + step <= query_length; position += step)
        query_prior = sz_overlap_f64x8_prefix_hash_step_skylake(query_prior, query + position,
                                                                query_prefix_hashes + position + 1);
    if (position != query_length)
        sz_overlap_f64x8_prefix_hash_step_tail_skylake(query_prior, query + position, query_length - position,
                                                       query_prefix_hashes + position + 1);
    position = walked;
    for (; position + step <= candidate_length; position += step)
        candidate_prior = sz_overlap_f64x8_prefix_hash_step_skylake(candidate_prior, candidate + position,
    ...
    for (sz_size_t width_index = 0; width_index != window_widths_count; ++width_index) {
        ...
        sz_size_t window = 0;
        for (; window + step <= windows; window += step)
            sz_overlap_f64x8_window_hash_step_skylake(candidate_prefix_hashes + window,
                                                      candidate_prefix_hashes + window + width, power,
                                                      window_hashes + window);
        if (window != windows)
            sz_overlap_f64x8_window_hash_step_tail_skylake(candidate_prefix_hashes + window,
                                                           candidate_prefix_hashes + window + width, power,
                                                           windows - window, window_hashes + window);
        sz_size_t const matches = sz_overlap_u32x16_btree_probe_skylake(&btree, window_hashes, windows);
