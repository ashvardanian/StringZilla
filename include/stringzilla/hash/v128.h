...
SZ_HELPER_INLINE v128_t sz_aes_linear_v128_(v128_t low_table, v128_t high_table, v128_t x) {
...
SZ_HELPER_AUTO v128_t sz_aes_gf4_mul_v128_(v128_t a, v128_t b) {
...
SZ_HELPER_INLINE sz_u128_vec_t sz_emulate_shuffle_epi8_v128_(sz_u128_vec_t state_vec, v128_t order) {
    sz_u128_vec_t result;
...
SZ_HELPER_AUTO void sz_hash_state_short_init_v128_(sz_hash_state_aligned_for_short_t_ *state, sz_u64_t seed) {
    sz_hash_state_short_init_serial_(state, seed);
}

SZ_HELPER_AUTO void sz_hash_state_short_update_v128_(sz_hash_state_aligned_for_short_t_ *state, sz_u128_vec_t block) {
...
SZ_HELPER_AUTO sz_u64_t sz_hash_state_short_finalize_v128_(sz_hash_state_aligned_for_short_t_ const *state,
                                                           sz_size_t length) {
...
SZ_HELPER_AUTO sz_hash_state_aligned_t_ sz_hash_state_load_v128_(sz_hash_state_t const *packed) {
...
SZ_HELPER_AUTO void sz_hash_state_store_v128_(sz_hash_state_t *packed, sz_hash_state_aligned_t_ const *state) {
    for (sz_size_t lane_index = 0; lane_index < 4; ++lane_index) {
        sz_size_t const offset = lane_index * 16;
...
SZ_HELPER_AUTO void sz_hash_state_update_v128_(sz_hash_state_aligned_t_ *state) {
    v128_t shuffle = wasm_v128_load(sz_hash_u8x16x4_shuffle_());
    for (sz_size_t lane_index = 0; lane_index < 4; ++lane_index) {
        ...
        state->sum.u128s[lane_index] = sz_emulate_shuffle_epi8_v128_(state->sum.u128s[lane_index], shuffle);
        state->sum.u128s[lane_index].v128 = wasm_i64x2_add(state->sum.u128s[lane_index].v128, ins_vec.v128);
    }
}

SZ_HELPER_AUTO sz_u64_t sz_hash_state_finalize_v128_(sz_hash_state_aligned_t_ state) {
    v128_t shuffle = wasm_v128_load(sz_hash_u8x16x4_shuffle_());
    ...
    sz_u128_vec_t aes0 = sz_emulate_aesenc_v128_(state.aes.u128s[0], ins0);
    sz_u128_vec_t aes1 = sz_emulate_aesenc_v128_(state.aes.u128s[1], ins1);
    sz_u128_vec_t aes2 = sz_emulate_aesenc_v128_(state.aes.u128s[2], ins2);
    sz_u128_vec_t aes3 = sz_emulate_aesenc_v128_(state.aes.u128s[3], ins3);
    sz_u128_vec_t sum0 = sz_emulate_shuffle_epi8_v128_(state.sum.u128s[0], shuffle);
    sz_u128_vec_t sum1 = sz_emulate_shuffle_epi8_v128_(state.sum.u128s[1], shuffle);
    sz_u128_vec_t sum2 = sz_emulate_shuffle_epi8_v128_(state.sum.u128s[2], shuffle);
    sz_u128_vec_t sum3 = sz_emulate_shuffle_epi8_v128_(state.sum.u128s[3], shuffle);
...
SZ_API_COMPTIME void sz_hash_state_update_v128(sz_hash_state_t *packed, sz_cptr_t text, sz_size_t length) {
    ...
    sz_hash_state_aligned_t_ state = sz_hash_state_load_v128_(packed);
 ...
 */
SZ_HELPER_AUTO sz_u64_t sz_hash_multiseed_replay_v128_(sz_u512_vec_t const *text_lanes, sz_size_t text_lanes_count,
                                                       sz_size_t length, sz_u64_t seed) {
    sz_align_(16) sz_hash_state_aligned_for_short_t_ state;
    sz_hash_state_short_init_v128_(&state, seed);
    for (sz_size_t lane_index = 0; lane_index < text_lanes_count; ++lane_index)
        sz_hash_state_short_update_v128_(&state, text_lanes->u128s[lane_index]);
    return sz_hash_state_short_finalize_v128_(&state, length);
}
...
SZ_HELPER_INLINE v128_t sz_sha256_rotr_v128_(v128_t x, int count) {
...
SZ_HELPER_INLINE v128_t sz_sha256_sigma0_lower_v128_(v128_t x) {
...
SZ_HELPER_INLINE v128_t sz_sha256_sigma1_lower_v128_(v128_t x) {
