...
SZ_INTERNAL sz_u64_t sz_hash_state_finalize_v128relaxed_(sz_hash_state_t const *state) {
    ...
    sz_u128_vec_t aes0 = sz_emulate_aesenc_v128relaxed_(aes_vecs[0], ins_vecs[0]);
    sz_u128_vec_t aes1 = sz_emulate_aesenc_v128relaxed_(aes_vecs[1], ins_vecs[1]);
    sz_u128_vec_t aes2 = sz_emulate_aesenc_v128relaxed_(aes_vecs[2], ins_vecs[2]);
    sz_u128_vec_t aes3 = sz_emulate_aesenc_v128relaxed_(aes_vecs[3], ins_vecs[3]);
    sz_u128_vec_t sum0 = sz_emulate_shuffle_epi8_v128relaxed_(sum_vecs[0], shuffle);
    sz_u128_vec_t sum1 = sz_emulate_shuffle_epi8_v128relaxed_(sum_vecs[1], shuffle);
    sz_u128_vec_t sum2 = sz_emulate_shuffle_epi8_v128relaxed_(sum_vecs[2], shuffle);
    sz_u128_vec_t sum3 = sz_emulate_shuffle_epi8_v128relaxed_(sum_vecs[3], shuffle);
...
SZ_INTERNAL sz_u64_t sz_hash_multiseed_replay_v128relaxed_(sz_u512_vec_t const *text_lanes, sz_size_t text_lanes_count,
    ...
    for (sz_size_t lane_index = 0; lane_index < text_lanes_count; ++lane_index)
        sz_hash_minimal_update_v128relaxed_(&state, text_lanes->u128s[lane_index]);
    return sz_hash_minimal_finalize_v128relaxed_(&state, length);
}
