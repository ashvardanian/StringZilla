 ...
 *
 *  The narrowest NEON tier: `u16` cells, so each `uint16x8_t` holds 8 lanes. The
 ...
 *
 *  One body per score-width, templated over the gap model and the locality and resolved with `if constexpr`, so a
 *  single hand-tuned kernel covers all four weighted `i16` cases. It runs the serial `tile_scorer` recurrence - the
 *  Gotoh `E`/`F` tracks under affine gaps, the zero clamp under Smith-Waterman - over an 8-lane `i16` layout, with
 *  the `classify16` candidate-class cache computed @b once before the row loop and a `lookup16` substitution gather
 *  whose low 8 `i8` lanes sign-extend to 8 `i16` cells.
 *
 *  The `F` track rides a previous/current row pair while `E` lives in a rolling per-lane register. Under
 *  Smith-Waterman the running maximum is updated only for columns within each lane's candidate length, so a shorter
 *  candidate's zero-padded tail cannot inflate its score.
 *
...
#pragma endregion NEON Inter Sequence Candidate Lanes
...
#pragma region Cross Product Scoring
    ...
    SZ_NOINLINE status_t score_range_(queries_type_ const &queries, candidates_type_ const &candidates,
        ...
        for (size_t cell_index = cell_begin; cell_index != cell_end;) {
            ...
            cross_cell_destination_t<value_t> group_destinations[myers_t::single_word_lanes_k];
...
#pragma region Cross Product Dispatch
  ...
  private:
    ...
    status_t cross_(queries_type_ const &queries, candidates_type_ const &candidates, strided_rows<value_type_> results,
                    cross_similarities_t cross_kind, cpu_specs_t const &specs) noexcept {
        if (corpus_is_ascii_<sz_find_byteset_neon>(queries) && corpus_is_ascii_<sz_find_byteset_neon>(candidates))
