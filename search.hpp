...
namespace av {
    ...
    /**
     * \brief This is faster than `strncmp()`.
     */
        ...
        size_t next_offset(span_t haystack, span_t needle) noexcept {
            ...
            for (; (h_ptr + 32) <= h_end; h_ptr += 32) {
                ...
                if (masks0 | masks1 | masks2 | masks3) {
                    // size_t full_match = prefixed_t {}.next_offset(span_t {(uint8_t *)h_ptr, needle.len + 32},
                    // needle); if (full_match != not_found_k)
                    //     return full_match + (h_ptr - haystack.data);
