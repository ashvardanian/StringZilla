/**
 *  @file c/stringzillas/find_many.cuh
 *  @brief Multi-pattern Aho-Corasick search shim (CPU + CUDA backends).
 *  @author Ash Vardanian
 *  @date August 6, 2026
 */
#ifndef STRINGZILLAS_SZS_FIND_MANY_CUH_
#define STRINGZILLAS_SZS_FIND_MANY_CUH_
...
/**
 *  @brief Allocates a `find_many_backends_t` holding the already-built @p engine_variant and publishes the
 *         opaque handle. The variant arrives pre-built, as `try_build` is fallible.
 */
template <typename engine_type_>
sz_status_t emplace_find_many_engine(szs_find_many_t *engine_punned, char const **error_message,
                                     engine_type_ engine_variant) noexcept {
    auto engine = new (std::nothrow)
        find_many_backends_t(std::in_place_type_t<engine_type_>(), std::move(engine_variant));
    if (!engine)
        return propagate_error(sz::status_t::bad_alloc_k, error_message, "Failed to allocate Find-Many engine");
    *engine_punned = reinterpret_cast<szs_find_many_t>(engine);
...
inline sz_status_t szs_find_many_count_(szs_find_many_t engine_punned, szs_device_scope_t device_punned,
    ...
    auto &engine = *reinterpret_cast<find_many_backends_t *>(engine_punned);
    auto &device = *reinterpret_cast<device_scope_t *>(device_punned);
    sz::span<size_t> const counts_span {counts, haystacks.size()};
    size_t matches_total = 0; // ? `szs_find_many_count` publishes only the per-haystack breakdown
...
/**
 *  @brief Locates every needle's occurrences in every haystack. @sa `szs_find_many_count_`.
 *
 ...
 */
template <typename haystacks_type_>
inline sz_status_t szs_find_many_find_(szs_find_many_t engine_punned, szs_device_scope_t device_punned,
                                       haystacks_type_ const &haystacks, szs_find_many_match_t *matches,
    ...
    static_assert(
        sizeof(szs::find_many_match_t) == sizeof(szs_find_many_match_t) &&
            offsetof(szs::find_many_match_t, haystack_index) == offsetof(szs_find_many_match_t, haystack_index) &&
            offsetof(szs::find_many_match_t, needle_index) == offsetof(szs_find_many_match_t, needle_index) &&
            offsetof(szs::find_many_match_t, byte_offset) == offsetof(szs_find_many_match_t, byte_offset) &&
            offsetof(szs::find_many_match_t, byte_length) == offsetof(szs_find_many_match_t, byte_length),
        "The C++ match must mirror the C ABI field for field, or this view would misreport matches");

    auto &engine = *reinterpret_cast<find_many_backends_t *>(engine_punned);
    auto &device = *reinterpret_cast<device_scope_t *>(device_punned);
    sz::span<szs::find_many_match_t> const matches_out {reinterpret_cast<szs::find_many_match_t *>(matches),
                                                        matches_capacity};
    *matches_found_out = 0;
...
SZ_API_RUNTIME sz_status_t szs_find_many_init(                                       //
    ...
    szs_find_many_t *engine_punned, char const **error_message) {
    ...
    szs::find_many_case_sensitivity_t const engine_case_sensitivity = case_sensitivity == szs_find_many_uncased_k
                                                                          ? szs::find_many_uncased_k
                                                                          : szs::find_many_cased_k;
    ...
    if (can_use_cuda) {
        szs::find_many_u32_cuda_t engine_variant;
        ...
        return emplace_find_many_engine<szs::find_many_u32_cuda_t>(engine_punned, error_message,
    ...
    if (can_use_parallel) {
        szs::find_many_u32_parallel_t engine_variant;
        ...
        return emplace_find_many_engine<szs::find_many_u32_parallel_t>(engine_punned, error_message,
                                                                       std::move(engine_variant));
    }

    szs::find_many_u32_serial_t engine_variant;
    sz::status_t const status = engine_variant.try_build(needles_container, engine_case_sensitivity);
    if (status != sz::status_t::success_k) return propagate_error(status, error_message);
    return emplace_find_many_engine<szs::find_many_u32_serial_t>(engine_punned, error_message,
                                                                 std::move(engine_variant));
}

SZ_API_RUNTIME sz_status_t szs_find_many_count(                      //
    szs_find_many_t engine_punned, szs_device_scope_t device_punned, //
    sz_sequence_t const *haystacks, sz_size_t *counts,               //
    char const **error_message) {

    sz_assert_(haystacks != nullptr && "Haystack collection cannot be null");
    return szs_find_many_count_(engine_punned, device_punned, sz_sequence_as_cpp_container_t {haystacks}, counts,
                                error_message);
}
...
SZ_API_RUNTIME sz_status_t szs_find_many(                                                 //
    ...
    sz_assert_(haystacks != nullptr && "Haystack collection cannot be null");
    return szs_find_many_find_(engine_punned, device_punned, sz_sequence_as_cpp_container_t {haystacks}, matches,
...
SZ_API_RUNTIME sz_status_t szs_find_many_count_u32tape(              //
    ...
    sz_assert_(haystacks != nullptr && "Haystack collection cannot be null");
    return szs_find_many_count_(engine_punned, device_punned, sz_sequence_u32tape_as_cpp_container_t {haystacks},
...
SZ_API_RUNTIME sz_status_t szs_find_many_count_u64tape(              //
    ...
    sz_assert_(haystacks != nullptr && "Haystack collection cannot be null");
    return szs_find_many_count_(engine_punned, device_punned, sz_sequence_u64tape_as_cpp_container_t {haystacks},
                                counts, error_message);
}
...
SZ_API_RUNTIME sz_status_t szs_find_many_u32tape(                                         //
    ...
    sz_assert_(haystacks != nullptr && "Haystack collection cannot be null");
    return szs_find_many_find_(engine_punned, device_punned, sz_sequence_u32tape_as_cpp_container_t {haystacks},
...
SZ_API_RUNTIME sz_status_t szs_find_many_u64tape(                                         //
    ...
    sz_assert_(haystacks != nullptr && "Haystack collection cannot be null");
    return szs_find_many_find_(engine_punned, device_punned, sz_sequence_u64tape_as_cpp_container_t {haystacks},
                               matches, matches_capacity, matches_found, error_message);
}

SZ_API_RUNTIME void szs_find_many_free(szs_find_many_t engine_punned) {
    sz_assert_(engine_punned != nullptr && "Engine must be initialized");
    auto *engine = reinterpret_cast<find_many_backends_t *>(engine_punned);
    delete engine;
}

#pragma endregion Find Many
}

#endif // STRINGZILLAS_SZS_FIND_MANY_CUH_
