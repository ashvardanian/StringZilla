/**
 *  @file c/stringzilla/utf8_case_fold.c
 *  @brief Per-domain dispatch shim for UTF-8 case folding.
 *  @author Ash Vardanian
 *  @date January 16, 2024
 *
 *  Split from the case-insensitive @b find/order shim (`utf8_case_insensitive.c`) so the cheap
 *  folding kernels compile as their own translation unit, in parallel with the heavy AVX-512 find.
 */
#include "dispatch.h"
#include <stringzilla/utf8_case_fold.h> // `sz_utf8_case_fold_*`
...
#if SZ_USE_NEON
    if (caps & sz_cap_neon_k) { impl->utf8_case_fold = sz_utf8_case_fold_neon; }
#endif
...
#if SZ_USE_LASX
    if (caps & sz_cap_lasx_k) { impl->utf8_case_fold = sz_utf8_case_fold_lasx; }
#endif

#if SZ_USE_POWERVSX
    if (caps & sz_cap_powervsx_k) { impl->utf8_case_fold = sz_utf8_case_fold_powervsx; }
#endif
}
