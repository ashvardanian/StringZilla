...
SZ_DISPATCH_INTERNAL void sz_dispatch_utf8_lines_update_(sz_capability_t caps) {
    ...
    impl->utf8_find_linewraps = sz_utf8_find_linewraps_serial;

#if SZ_USE_ICELAKE
    if (caps & sz_cap_icelake_k) { impl->utf8_find_linewraps = sz_utf8_find_linewraps_icelake; }
#endif
}
...
SZ_DYNAMIC sz_size_t sz_utf8_find_linewraps(sz_cptr_t text, sz_size_t length, sz_size_t *line_starts,
    ...
    return sz_dispatch_table.utf8_find_linewraps(text, length, line_starts, line_lengths, lines_capacity,
                                                 bytes_consumed);
}
