/**
 *  @file c/stringzilla/utf8_case_insensitive.c
 *  @brief Per-domain dispatch shim for case-insensitive UTF-8 search & ordering.
 *  @author Ash Vardanian
 *  @date January 16, 2024
 *
 *  Isolated from the folding shim (`utf8_case_fold.c`) because the AVX-512 per-script find kernels in
 *  `utf8_case_insensitive/icelake.h` are by far the heaviest single compilation in the core; keeping
 *  them in their own translation unit lets the rest of the UTF-8 case domain build in parallel.
 */
#include "dispatch.h"
#include <stringzilla/utf8_case_insensitive.h> // `sz_utf8_case_insensitive_{find,order}_*`
...
SZ_DISPATCH_INTERNAL void sz_dispatch_utf8_case_insensitive_update_(sz_capability_t caps) {
...
#if SZ_USE_NEON
    if (caps & sz_cap_neon_k) { impl->utf8_case_insensitive_find = sz_utf8_case_insensitive_find_neon; }
#endif

#if SZ_USE_V128
    if (caps & sz_cap_v128_k) {
        impl->utf8_case_insensitive_find = sz_utf8_case_insensitive_find_v128;
        impl->utf8_case_insensitive_order = sz_utf8_case_insensitive_order_v128;
    }
#endif
    ...
    if (caps & sz_cap_rvv_k) {
        ...
        impl->utf8_case_insensitive_order = sz_utf8_case_insensitive_order_rvv;
    }
#endif

#if SZ_USE_LASX
    if (caps & sz_cap_lasx_k) {
        impl->utf8_case_insensitive_find = sz_utf8_case_insensitive_find_lasx;
        impl->utf8_case_insensitive_order = sz_utf8_case_insensitive_order_lasx;
    }
#endif

#if SZ_USE_POWERVSX
    if (caps & sz_cap_powervsx_k) {
        impl->utf8_case_insensitive_find = sz_utf8_case_insensitive_find_powervsx;
        impl->utf8_case_insensitive_order = sz_utf8_case_insensitive_order_powervsx;
    }
#endif
}
