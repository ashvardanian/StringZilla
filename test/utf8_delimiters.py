...
def test_utf8_delimiters():
    ...
    assert list(sz.utf8_delimiters("Hi, world")) == [Str("Hi"), Str(""), Str("world")]

    # skip_empty drops the empty between adjacent delimiters.
    assert list(sz.utf8_delimiters("Hi, world", skip_empty=True)) == [Str("Hi"), Str("world")]

    # A single delimiter yields two segments, no empty.
    assert list(sz.utf8_delimiters("a.b")) == [Str("a"), Str("b")]

    # Empty string yields one empty Str (mirrors utf8_tokens).
    assert list(sz.utf8_delimiters("")) == [Str("")]

    # No delimiters: the whole string is one segment.
    assert list(sz.utf8_delimiters("hello")) == [Str("hello")]

    # Unicode punctuation: U+2014 EM DASH (Pd) is a delimiter.
    assert list(sz.utf8_delimiters("a—b", skip_empty=True)) == [Str("a"), Str("b")]

    # Unicode punctuation: CJK U+3002 IDEOGRAPHIC FULL STOP (Po).
    assert list(sz.utf8_delimiters("日本。語", skip_empty=True)) == [Str("日本"), Str("語")]

    # Emoji symbol (So) is a delimiter.
    assert list(sz.utf8_delimiters("a\U0001f600b", skip_empty=True)) == [Str("a"), Str("b")]

    # Method form on Str matches the module function.
    assert list(sz.Str("Hi, world").utf8_delimiters(skip_empty=True)) == [Str("Hi"), Str("world")]

    # Letters and digits are NOT delimiters.
    assert list(sz.utf8_delimiters("abc123")) == [Str("abc123")]
...
def test_utf8_delimiters_safety(seed_value: int):
    ...
    for raw in adversarial_utf8_inputs(rng):
        segments = list(sz.utf8_delimiters(raw))
